...
fun MainTeacherScreen(navController2: NavHostController){
    ...
    ModalNavigationDrawer(
            ...
            ModalDrawerSheet(
                ...
                navigationMenuItems.forEach() { item ->
                    ...
                    NavigationDrawerItem(
                        ...
                        selected = currentRoute == item.routeTeacher,
                        onClick = {
                            scope.launch {
                                drawerState.apply {
                                    close()
        ...
        Scaffold(
            ...
            bottomBar = { MyTeacherBottomBar(navController, navigationBottomItems) },
            content = { padding ->
                Column(
                    modifier = Modifier
                        .padding(padding)
        ...
        content = {
            NavigationBar {
                ...
                navigationBottomItems.forEach { item ->
                    NavigationBarItem(
                        selected = currentRoute == item.routeTeacher,
                        onClick = { navController.navigate(item.routeTeacher) },
                        icon = {
                            Icon(
                                painter = painterResource(id = item.icon),
                                contentDescription = item.title,
                                tint = if (currentRoute == item.routeTeacher) Color(0xFF495ECA) else Color(
                                    0xFF8B97A8
                                )
                            )
                        }
